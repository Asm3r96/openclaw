...
- CLI/Status: fix `/status` overview row showing the configured model instead of the actual model when a fallback is active; the Sessions row now reflects the most recent session's runtime model.
